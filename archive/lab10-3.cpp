...
int setUpHospital(TreeNode *root) {
    int num = 0;
    int downFalse = false;
    if (root)
    {
        if(root->lc) {
            num += setUpHospital(root->lc);
            if(!root->lc->have) {
                downFalse = true;
            }
        }
        if(root->rc) {
            num += setUpHospital(root->rc);
            if(!root->rc->have) {
                downFalse = true;
            }
        }
        ...
        if(downFalse) {
            ...
            if(root->parent) {
                root->parent->have = true;
            }
        }

        if(!root->have && root == head) {
            num++;
            root->have = true;
            if(root->lc) {
                root->lc->have = true;
